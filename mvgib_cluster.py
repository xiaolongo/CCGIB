...
from utils import k_means_cluster, view_generator  # attack_edge


def logger(info):
    epoch = info['epoch']
    train_loss, test_NMI, test_ARI = info['train_loss'], info['NMI'], info[
        'ARI']
    print(f'{epoch:03d}: Train Loss: {train_loss:.3f}, NMI: {test_NMI:.4f} '
          f' ARI: {test_ARI:.4f}')
...
def test(model, loader, num_classes, device):
    ...
    NMI, ARI = k_means_cluster(z, y, num_classes=num_classes)  #
    return NMI, ARI


def cross_validation(dataset, model, epochs, batch_size, lr, lr_decay_factor,
                     lr_decay_step_size, weight_decay, device, beta, gamma, num_classes):
    model.to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        NMI, ARI = test(model=model, loader=loader, num_classes=num_classes, device=device)
